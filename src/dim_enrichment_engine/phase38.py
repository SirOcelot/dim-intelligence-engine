...
def build_dim_payload(build_payload: dict, stat_payload: dict) -> dict:
    return {
        ...
        'notes': build_payload.get('notes', []) + ['Warmind export scaffold', 'DIM import string not wired yet'],
...
def build_dim_export_md(dim_payload: dict) -> str:
    ...
    lines.extend(['', '## Export Status', '- This is a Warmind DIM-focused export scaffold, ready for later conversion into a true DIM import string.', '- Item hashes and socket/mod packaging are not wired yet.'])
    return '\n'.join(lines)


def build_dim_share_scaffold(dim_payload: dict) -> dict:
    items = []
    for slot, weapon in zip(['kinetic', 'energy', 'power'], dim_payload.get('weapons', [])):
        items.append({'slot': slot, 'name': weapon, 'hash': None, 'id': None, 'equipped': True})
    items.append({'slot': 'armor_exotic', 'name': dim_payload.get('armor_exotic'), 'hash': None, 'id': None, 'equipped': True})
    return {
        'app': 'Warmind',
        'engine': 'Destiny Intelligence Engine',
        'type': 'dim-share-scaffold',
        ...
        'status': {
            'dimImportReady': False,
            'shareLinkReady': False,
            'needsItemHashes': True,
            'needsDimLoadoutSchemaWiring': True,
        },
    }


def build_dim_share_md(share_payload: dict) -> str:
    lines = ['# DIM Share Scaffold', '']
    lines.append(f"- Name: **{share_payload.get('name', 'Warmind Loadout')}**")
    lines.append(f"- Class: **{share_payload.get('classType', 'Hunter')}**")
    lines.append(f"- Subclass: **{share_payload.get('subclass', 'unknown')}**")
    lines.extend(['', '## Included Items'])
    for item in share_payload.get('items', []):
        lines.append(f"- **{item.get('slot')}**: {item.get('name')} | hash: {item.get('hash')} | id: {item.get('id')}")
    lines.extend(['', '## Parameters'])
    params = share_payload.get('parameters', {})
    ...
    lines.extend(['', '## Next Wiring Needed', '- Resolve item hashes and item instance IDs from live inventory data.', '- Map this scaffold to DIM\'s true loadout/share document format.', '- Generate a real share/import object after schema wiring is complete.'])
    return '\n'.join(lines)
...
def generate_reports(weapons: Path, armor: Path, loadouts: Path | None, output_dir: Path, use_bungie: bool) -> None:
    ...
    dim_payload = build_dim_payload(build_payload, stat_payload); share_payload = build_dim_share_scaffold(dim_payload)
    ...
    write(output_dir / 'DIM Share Scaffold.md', build_dim_share_md(share_payload))
    ...
    (output_dir / 'dim_share_scaffold.json').write_text(json.dumps(share_payload, indent=2), encoding='utf-8')
    canonical_payload = {'app': 'Warmind', 'engine': 'Destiny Intelligence Engine', 'stat_optimization': stat_payload, 'build_recommendation': build_payload, 'dim_export': dim_payload, 'dim_share_scaffold': share_payload}
...
def main() -> None:
    parser = argparse.ArgumentParser(description='Warmind Destiny Intelligence Engine Phase 3.8')
