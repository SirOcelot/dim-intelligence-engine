...
def select_adaptive_build(owned: set[str], issue: str) -> dict[str, str]:
    ...
    priorities = CANDIDATE_PRIORITIES[adaptive_profile]
...
def build_adaptation_reason(issue: str, before: dict[str, str], after: dict[str, str]) -> list[str]:
    reasons = list(ISSUE_PROFILES[issue]["notes"])
    for slot in ["kinetic", "energy", "power", "armor_exotic"]:
        if before.get(slot) != after.get(slot):
            reasons.append(f"Changed {slot} from {before.get(slot)} to {after.get(slot)} to address {issue.replace('_', ' ')}.")
    return reasons
...
def render_md(issue: str, baseline_profile: str, adaptive_profile: str, before_build: dict[str, str], after_build: dict[str, str], before_scores: dict[str, float], after_scores: dict[str, float], comparison: dict[str, Any], reasons: list[str], targets: dict[str, dict[str, float]]) -> str:
    lines = [
        "# Warmind Adaptive Intelligence",
        "",
        f"- Issue: **{issue}**",
    ...
    lines.extend(["", "## Adaptive Stat Targets"])
    for stat in STAT_ORDER:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Warmind Phase 13.1 adaptive intelligence engine")
    ...
    parser.add_argument("--issue", required=True, choices=sorted(ISSUE_PROFILES.keys()))
    ...
    before_build = select_adaptive_build(owned, "champion_failures" if baseline_profile == "gm" else baseline_profile if baseline_profile in ISSUE_PROFILES else "dying_too_much")
    # normalize baseline using same candidate system but profile-only
    if baseline_profile == "gm":
        before_build = {
            "kinetic": CANDIDATE_PRIORITIES["gm"]["kinetic"][0] if CANDIDATE_PRIORITIES["gm"]["kinetic"][0] in owned else next((x for x in CANDIDATE_PRIORITIES["gm"]["kinetic"] if x in owned), "No clear pick"),
            "energy": next((x for x in CANDIDATE_PRIORITIES["gm"]["energy"] if x in owned), "No clear pick"),
            "power": next((x for x in CANDIDATE_PRIORITIES["gm"]["power"] if x in owned), "No clear pick"),
            "armor_exotic": next((x for x in CANDIDATE_PRIORITIES["gm"]["armor_exotic"] if x in owned), "No clear pick"),
            "subclass": "Nightstalker",
        }
    elif baseline_profile == "dps":
        before_build = {
            "kinetic": next((x for x in CANDIDATE_PRIORITIES["dps"]["kinetic"] if x in owned), "No clear pick"),
            "energy": next((x for x in CANDIDATE_PRIORITIES["dps"]["energy"] if x in owned), "No clear pick"),
            "power": next((x for x in CANDIDATE_PRIORITIES["dps"]["power"] if x in owned), "No clear pick"),
            "armor_exotic": next((x for x in CANDIDATE_PRIORITIES["dps"]["armor_exotic"] if x in owned), "No clear pick"),
            "subclass": "Nightstalker",
        }
    else:
        before_build = {
            "kinetic": next((x for x in CANDIDATE_PRIORITIES["survivability"]["kinetic"] if x in owned), "No clear pick"),
            "energy": next((x for x in CANDIDATE_PRIORITIES["survivability"]["energy"] if x in owned), "No clear pick"),
            "power": next((x for x in CANDIDATE_PRIORITIES["survivability"]["power"] if x in owned), "No clear pick"),
            "armor_exotic": next((x for x in CANDIDATE_PRIORITIES["survivability"]["armor_exotic"] if x in owned), "No clear pick"),
            "subclass": "Nightstalker",
        }

    adaptive_profile = ISSUE_PROFILES[args.issue]["profile"]
    adaptive_targets = adjust_stat_targets(current, adaptive_profile, args.issue)
    baseline_targets = adjust_stat_targets(current, baseline_profile, "champion_failures" if baseline_profile == "gm" else args.issue)

    after_build = select_adaptive_build(owned, args.issue)
    before_scores = score_build(before_build, baseline_targets, baseline_profile, None)
    after_scores = score_build(after_build, adaptive_targets, adaptive_profile, args.issue)
    comparison = compare_scores(before_scores, after_scores)
    reasons = build_adaptation_reason(args.issue, before_build, after_build)
    ...
    (out / "phase131_adaptive.json").write_text(json.dumps(payload, indent=2), encoding="utf-8")
    write(out / "Phase131 Adaptive.md", render_md(args.issue, baseline_profile, adaptive_profile, before_build, after_build, before_scores, after_scores, comparison, reasons, adaptive_targets))
