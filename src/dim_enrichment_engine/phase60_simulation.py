...
def weakest_link(stat_info: dict) -> dict:
    deltas = {}
    for stat in STAT_ORDER:
...
def simulate_swap(context: dict, swap_out: str, swap_in: str, owned: set[str]) -> dict:
    ...
    if slot_found is None:
        return {
            "success": False,
            "summary": f"Warmind could not find {swap_out} in the recommended build, so no swap simulation was performed.",
        }
    if swap_in not in owned:
        return {
            "success": False,
            "summary": f"Warmind could not confirm ownership of {swap_in}, so the swap would be theoretical rather than vault-aware.",
        }
    old = rb[slot_found]
    rb[slot_found] = swap_in
    impact = []
    if slot_found == "energy":
        if old == "Le Monarque" and swap_in == "Graviton Lance":
            impact.append("You gain easier neutral add-clear feel, but lose overload-focused utility and some long-range safety.")
            verdict = "Worse for champion-heavy GMs, more comfortable for lower-pressure content."
        elif old == "Le Monarque" and swap_in == "Outbreak Perfected":
            impact.append("You gain better ammo-efficient primary damage, but lose direct overload identity in this shell.")
            verdict = "Viable when champion utility is covered elsewhere."
        else:
            impact.append(f"This changes the energy role from {old} to {swap_in}. Warmind would re-evaluate champion coverage and encounter fit.")
            verdict = "Situational."
    elif slot_found == "power":
        if old == "Leviathan's Breath" and swap_in == "Thunderlord":
            impact.append("You gain easier sustained damage and comfort, but lose some champion utility and safer burst windows.")
            verdict = "Better for ease, worse for harder utility checks."
        else:
            impact.append(f"This changes the heavy role from {old} to {swap_in}. Warmind would re-evaluate boss damage and champion utility.")
            verdict = "Situational."
    elif slot_found == "armor_exotic":
        if old == "Gyrfalcon's Hauberk" and swap_in == "Cyrtarachne's Facade":
            impact.append("You trade invis and volatile loop power for a more DR-oriented stabilization shell.")
            verdict = "Safer in some neutral play, weaker in void-synergy pressure."
        else:
            impact.append(f"This changes the build identity from {old} to {swap_in}.")
            verdict = "Needs activity-specific review."
    else:
        impact.append(f"This changes the {slot_found} slot from {old} to {swap_in}.")
        verdict = "Situational."
    return {
        "success": True,
        "slot": slot_found,
        ...
        "impact": impact,
        "verdict": verdict,
        "summary": f"Swap simulation: {old} -> {swap_in}. {verdict}",
    }
...
def simulate_stat_shift(context: dict, stat: str, delta: float) -> dict:
    ...
    target = context["stat_targets"][stat_norm]
    current = target["current"]
    new_value = current + delta
    low = target["target_low"]
    high = target["target_high"]
    if new_value < low:
        verdict = "This would move the stat further below target and likely make the build worse in practice."
    elif new_value > high:
        verdict = "This would push the stat above the target band and may steal budget from more important stats."
    else:
        verdict = "This would move the stat into or within the target band and is directionally healthy."
    return {
        "success": True,
        "stat": stat_norm,
        "current": current,
        ...
        "target_low": low,
        "target_high": high,
        "summary": f"Stat simulation: {stat_norm} {current:.0f} -> {new_value:.0f}. {verdict}",
        "verdict": verdict,
    }


def parse_question(question: str, context: dict, owned: set[str]) -> str:
    q = question.strip().lower()

    for entry in context["decision_log"]:
        if entry["selected"].lower() in q and ("why" in q or "chosen" in q or "pick" in q):
            return f"{entry['selected']} was selected for {entry['slot']} because {entry['reason']}."
        for rejected in entry["rejected"]:
            if rejected.lower() in q and ("why not" in q or "instead of" in q or "swap" in q):
                return f"{rejected} was not selected because Warmind preferred {entry['selected']} for {entry['slot']}, since {entry['reason']}."

    if "weakest" in q or "weak point" in q:
        return context["weakest_link"]["summary"]
    if "stat" in q and ("first" in q or "change" in q or "fix" in q):
        wl = context["weakest_link"]
        return f"Change {wl['weakest_stat']} first. {wl['summary']}"
    if "farm" in q or "chase" in q or "next" in q:
        return f"Best farm targets to improve this shell next are: {', '.join(context['farm_targets']) if context['farm_targets'] else 'no high-priority chase gaps detected'}."
    if "best swap" in q or ("swap" in q and "best" in q):
        return f"Best owned swap candidates after the current recommendation are: {', '.join(context['best_swap_candidates']) if context['best_swap_candidates'] else 'no strong owned alternatives detected beyond the current recommendation'}."
    if "survivability" in q or "survive" in q or "tankier" in q:
        return "For more survivability, keep the core safe shell and move budget out of excess Weapons into Health and Class first."
    if "dps" in q or "damage" in q:
        return "For more DPS, keep the survivability floor intact, then push Weapons and Super toward the top of their target bands and consider burst-oriented swaps only if the activity allows it."
    ...
    if "swap" in q and " for " in q:
        ...
        sim = simulate_swap(context, swap_out, swap_in, owned)
        return sim["summary"]
    ...
    if ("drop " in q or "add " in q or "increase " in q or "reduce " in q) and any(stat.lower() in q for stat in STAT_ORDER):
        ...
        if "drop" in q or "reduce" in q:
            ...
            mode = 1
        if mode is None or delta == 0:
            return "Warmind saw a stat-change question, but could not confidently parse the amount. Try something like 'drop 20 weapons' or 'add 20 health'."
        for stat in STAT_ORDER:
            if stat.lower() in q:
                sim = simulate_stat_shift(context, stat, mode * abs(delta))
                return sim["summary"]

    if "build" in q and "why" in q:
        rb = context["recommended_build"]
        return f"Warmind selected {rb['kinetic']}, {rb['energy']}, {rb['power']}, and {rb['armor_exotic']} because the current profile is {context['profile']} and the system confidence is {context['confidence']}."

    return "Warmind did not find a tight match for that question yet. Try asking about why an item was selected, why an item was not selected, what stat to change first, best swap, weakest link, survivability, DPS, what to farm next, swap X for Y, or drop/add a stat amount."


def render_simulation_md(context: dict, question: str | None, answer: str | None) -> str:
    lines = ["# Warmind Simulation", "", "## Recommended Build"]
    rb = context["recommended_build"]
    lines.append(f"- Kinetic: **{rb['kinetic']}**")
    lines.append(f"- Energy: **{rb['energy']}**")
    lines.append(f"- Power: **{rb['power']}**")
    lines.append(f"- Exotic Armor: **{rb['armor_exotic']}**")
    lines.append(f"- Subclass: **{rb['subclass']}**")
    lines.extend(["", "## Quick Tuning"])
    lines.append(f"- Weakest link: **{context['weakest_link']['weakest_stat']}**")
    lines.append(f"- Confidence: **{context['confidence']}**")
    if question and answer:
        lines.extend(["", "## Interactive Result", f"- **Question:** {question}", f"- **Answer:** {answer}"])
    else:
        lines.extend(["", "## Example Questions", "- why not graviton lance", "- swap le monarque for graviton lance", "- drop 20 weapons", "- add 20 health", "- what should I farm next"])
    return "\n".join(lines)


def main() -> None:
    parser = argparse.ArgumentParser(description="Warmind simulation engine")
    ...
    context = build_qa_context(owned, current, stat_info, reasoning)
    answer = parse_question(args.question, context, owned) if args.question else None

    out = Path(args.output_dir)
    out.mkdir(parents=True, exist_ok=True)
    write(out / "Simulation.md", render_simulation_md(context, args.question, answer))
    if answer:
        write(out / "Simulation Answer.txt", answer)
    (out / "simulation_context.json").write_text(json.dumps({"context": context, "question": args.question, "answer": answer}, indent=2), encoding="utf-8")
