...
def build_gm_meta(owned_set: set[str], current: dict[str, str]) -> str:
    ...
    champs = champion_lists(owned_set)
...
def build_build_recommendation(owned_set: set[str], current: dict[str, str], stat_payload: dict) -> tuple[str, dict]:
    ...
    reasons = []
    if build['kinetic'] == 'Buried Bloodline':
        reasons.append('- Buried Bloodline gives you one of the safest utility/survivability shells you own.')
    elif build['kinetic'] == 'Witherhoard':
        reasons.append('- Witherhoard gives passive damage and safe area control.')
    if build['energy'] == 'Le Monarque':
        reasons.append('- Le Monarque gives safe ranged pressure and strong overload coverage.')
    elif build['energy'] == 'Outbreak Perfected':
        reasons.append('- Outbreak Perfected gives reliable primary damage and excellent ammo economy.')
    if build['heavy'] == "Leviathan's Breath":
        reasons.append("- Leviathan's Breath is one of your safest heavy anchors for champion and boss utility.")
    elif build['heavy'] == 'Thunderlord':
        reasons.append('- Thunderlord is low-friction, easy damage with good ad-control value.')
    if build['armor_exotic'] == "Gyrfalcon's Hauberk":
        reasons.append('- Gyrfalcon gives a practical invis/volatile loop that fits blind endgame better than greedier shells.')
    elif build['armor_exotic'] == "Cyrtarachne's Facade":
        reasons.append('- Cyrtarachne helps stabilize survivability through DR instead of stat greed.')
    ...
    export_payload = {
        ...
        'activity': 'GM',
        'class': 'Hunter',
        'weapons': [build['kinetic'], build['energy'], build['heavy']],
        'armor_exotic': build['armor_exotic'],
        'subclass': build['subclass'],
        'stat_targets': stat_payload.get('targets', {}),
        'tags': ['gm', 'hunter', 'survivability-first'],
...
def build_dim_export_md(dim_payload: dict) -> str:
    lines = ['# DIM Export', '']
    lines.append('## Warmind Package')
    lines.append(f"- App: **{dim_payload.get('app', 'Warmind')}**")
    lines.append(f"- Engine: **{dim_payload.get('engine', 'Destiny Intelligence Engine')}**")
    lines.append(f"- Profile: **{dim_payload.get('profile', 'unknown')}**")
    lines.append(f"- Activity: **{dim_payload.get('activity', 'unknown')}**")
    lines.append(f"- Class: **{dim_payload.get('class', 'unknown')}**")
    lines.append(f"- Subclass: **{dim_payload.get('subclass', 'unknown')}**")
    lines.extend(['', '## Weapons'])
    for weapon in dim_payload.get('weapons', []):
        lines.append(f'- **{weapon}**')
    lines.extend(['', '## Armor'])
    lines.append(f"- Exotic Armor: **{dim_payload.get('armor_exotic', 'unknown')}**")
    lines.extend(['', '## Target Stats'])
    for stat in STAT_DISPLAY_ORDER:
    ...
    lines.extend(['', '## Notes'])
    for note in dim_payload.get('notes', []):
        lines.append(note if note.startswith('- ') else f'- {note}')
    lines.extend(['', '## Export Status'])
    lines.append('- This is a Warmind DIM-focused export scaffold, ready for later conversion into a true DIM import string.')
    lines.append('- Item hashes and socket/mod packaging are not wired yet.')
    return '\n'.join(lines)
...
def build_dim_payload(build_payload: dict, stat_payload: dict) -> dict:
    return {
        ...
        'profile': build_payload.get('profile', 'gm'),
        'activity': build_payload.get('activity', 'GM'),
        'class': build_payload.get('class', 'Hunter'),
        'subclass': build_payload.get('subclass', 'Nightstalker'),
        'weapons': build_payload.get('weapons', []),
        'armor_exotic': build_payload.get('armor_exotic', 'Unknown'),
        'stat_targets': build_payload.get('stat_targets', stat_payload.get('targets', {})),
        'tags': build_payload.get('tags', []),
        'notes': build_payload.get('notes', []) + [
            'Warmind export scaffold',
            'DIM import string not wired yet',
        ],
        'source': {
            'build_recommendation': 'Build Recommendation.md',
            'stat_optimization': 'Stat Optimization.md',
        },
    }
...
def generate_reports(weapons: Path, armor: Path, loadouts: Path | None, output_dir: Path, use_bungie: bool) -> None:
    ...
    stat_md, stat_payload = build_stat_optimization(current_hunter)
    build_md, build_payload = build_build_recommendation(owned_set, current_hunter, stat_payload)
    dim_payload = build_dim_payload(build_payload, stat_payload)
    ...
    canonical_payload = {
        'app': 'Warmind',
        'engine': 'Destiny Intelligence Engine',
        'stat_optimization': stat_payload,
        'build_recommendation': build_payload,
        'dim_export': dim_payload,
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(description='Warmind Destiny Intelligence Engine Phase 3.7')
