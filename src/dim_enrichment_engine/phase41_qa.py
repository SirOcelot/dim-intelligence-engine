...
def weakest_link(current: dict[str, str], stat_info: dict) -> dict:
    current_stats = {stat: stat_info["targets"][stat]["current"] for stat in STAT_ORDER}
    deltas = {}
    for stat in STAT_ORDER:
...
def build_qa_context(owned: set[str], current: dict[str, str], stat_info: dict, reasoning: dict) -> dict:
    ...
    qa = {
        ...
        "qa_answers": {
            "why_selected": {
                entry["selected"]: f"{entry['selected']} was selected for {entry['slot']} because {entry['reason']}."
                for entry in reasoning["decision_log"]
            },
            "why_not_selected": {
                rejected: f"{rejected} was not selected because Warmind preferred {entry['selected']} for {entry['slot']}, since {entry['reason']}."
                for entry in reasoning["decision_log"] for rejected in entry["rejected"]
            },
...
def render_build_qa_md(context: dict) -> str:
    ...
    if context["qa_answers"]["why_not_selected"]:
        for item, answer in sorted(context["qa_answers"]["why_not_selected"].items()):
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Warmind Build Q&A layer")
    ...
    write(out / "Build QA.md", render_build_qa_md(qa_context))
    (out / "qa_context.json").write_text(json.dumps(qa_context, indent=2), encoding="utf-8")
    (out / "recommendation_export.json").write_text(json.dumps({"qa_context": qa_context, "reasoning": reasoning, "stat_optimization": stat_info}, indent=2), encoding="utf-8")
